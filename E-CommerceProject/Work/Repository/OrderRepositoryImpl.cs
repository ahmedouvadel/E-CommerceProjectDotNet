...
namespace E_CommerceProject.Work.Repository
{
    public class OrderRepositoryImpl : IOrderRepository
    {
        ...
        public OrderRepositoryImpl(ECommerceDB context, ShoppingCart shoppingCart)
        {
            _context = context;
            ...
            foreach (var item in _shoppingCart.ShoppingCartItems)
            {
                orderDetails.Add(
                    new OrderDetail
                    {
                        ...
                        Amount = Math.Min(item.Amount, (byte)item.Product.InStock),
                        Price = item.Product.Price,
                        Product = item.Product
                    });
                //_context.Update(item.Product);
                //item.Product.InStock = Math.Max((byte)(item.Product.InStock - item.Amount), 0);
            }
